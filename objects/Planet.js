 ...
 */
function buildPolyVerts(sides, r) {
    const pts = [];
...
function buildBumpyVerts(r, waves) {
    ...
    for (let i = 0; i < steps; i++) {
        ...
        for (const [freq, amp, phase] of waves)
            br += base * amp * Math.sin(freq * angle + phase);
        pts.push({ x: Math.cos(angle) * br, y: Math.sin(angle) * br });
    }
    return pts;
}
function buildRingPhysicsVerts(r, ringA, ringB) {
    ...
    const spherePt = (θ) => ({ x: r * Math.cos(θ), y: r * Math.sin(θ) });
    // Right ring arc (θ: −trans → +trans, through 0)
    for (let i = 0; i <= RS; i++)
        pts.push(ringPt(-trans + (2 * trans) * i / RS));
    // Top sphere arc (θ: +trans → π−trans)
    for (let i = 1; i < SS; i++)
        pts.push(spherePt(trans + (Math.PI - 2 * trans) * i / SS));
    // Left ring arc (θ: π−trans → π+trans, through π)
    for (let i = 0; i <= RS; i++)
        pts.push(ringPt(Math.PI - trans + (2 * trans) * i / RS));
    // Bottom sphere arc (θ: π+trans → 2π−trans)
    for (let i = 1; i < SS; i++)
        pts.push(spherePt(Math.PI + trans + (Math.PI - 2 * trans) * i / SS));
    return pts;
}
export class Planet extends Phaser.Physics.Matter.Image {
    constructor(scene, x, y, planetId) {
