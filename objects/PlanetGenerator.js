import { PLANETS, SHAPE_WAVES } from '../config/planets';
...
export function generatePlanetTextures(scene) {
    for (const config of PLANETS) {
        if (config.texture === 'supernova')
            continue;
...
export function generateSaturnRings(scene, config) {
    drawSaturnRingsTexture(scene, config);
}
// ─── main planet draw ──────────────────────────────────────────────────────
// ─── polygon / bumpy shape helpers ────────────────────────────────────────
function filledPoly(g, cx, cy, r, sides, color, alpha) {
    const pts = [];
...
function bumpedPoly(g, cx, cy, r, waves, color, alpha) {
    const steps = 32; // enough points to look smooth
    const pts = [];
    for (let i = 0; i < steps; i++) {
        const angle = (i / steps) * Math.PI * 2;
        let br = r;
        for (const [freq, amp, phase] of waves)
            br += r * amp * Math.sin(freq * angle + phase);
...
// ─── main planet draw ──────────────────────────────────────────────────────
function drawPlanet(scene, config, baseColor, shadowColor, detailColor) {
    const r = config.radius;
    const g = scene.make.graphics({ x: 0, y: 0 });
    const cx = r, cy = r;
    const ol = Math.max(3, r * 0.09);
    const ir = r - ol * 0.6;
    const waves = SHAPE_WAVES[config.shape];
    ...
    if (waves) {
        ...
        // ── circle planet ───────────────────────────────────────────────────────
        if (config.id === 9)
            drawSunRays(g, cx, cy, ir, detailColor);
...
// ─── surface details ───────────────────────────────────────────────────────
function drawSurface(g, id, cx, cy, r, base, shadow, detail) {
    switch (id) {
        ...
        case 2: { // Venus — horizontal wavy bands
            ...
            for (const yOff of [-0.40, -0.05, 0.35]) {
                ...
                g.beginPath();
                g.moveTo(cx - hw, cy + dy);
                g.lineTo(cx + hw, cy + dy);
                g.strokePath();
            }
            break;
        }
...
// ─── Sun rays ──────────────────────────────────────────────────────────────
function drawSunRays(g, cx, cy, ir, color) {
    const rayCount = 16;
...
// ─── kawaii faces ──────────────────────────────────────────────────────────
function drawFace(g, id, cx, cy, r) {
    ...
    switch (id) {
        case 0:
            drawFacePluto(g, cx, cy, r, eLX, eRX, eY, eR, sw);
            break;
        case 1:
            drawFaceMercury(g, cx, cy, r, eLX, eRX, eY, eR, sw);
            break;
        case 2:
            drawFaceVenus(g, cx, cy, r, eLX, eRX, eY, eR, sw);
            break;
        case 3:
            drawFaceEarth(g, cx, cy, r, eLX, eRX, eY, eR, sw);
            break;
        case 4:
            drawFaceMars(g, cx, cy, r, eLX, eRX, eY, eR, sw);
            break;
        case 5:
            drawFaceJupiter(g, cx, cy, r, eLX, eRX, eY, eR, sw);
            break;
        case 6:
            drawFaceSaturn(g, cx, cy, r, eLX, eRX, eY, eR, sw);
            break;
        case 7:
            drawFaceUranus(g, cx, cy, r, eLX, eRX, eY, eR, sw);
            break;
        case 8:
            drawFaceNeptune(g, cx, cy, r, eLX, eRX, eY, eR, sw);
            break;
        case 9:
            drawFaceSun(g, cx, cy, r, eLX, eRX, eY, eR, sw);
            break;
    }
}
// ── shared helpers ────────────────────────────────────────────────────────
function cheeks(g, cx, cy, r) {
    g.fillStyle(0xFF8899, 0.50);
    g.fillEllipse(cx - r * 0.42, cy + r * 0.26, r * 0.52, r * 0.26);
    g.fillEllipse(cx + r * 0.42, cy + r * 0.26, r * 0.52, r * 0.26);
}
// Closed happy eye: upward arc (~~)
function closedEyeHappy(g, ex, ey, eR, sw) {
...
function closedEyeSleepy(g, ex, ey, eR, sw) {
...
function closedEyeWink(g, ex, ey, eR, sw) {
    g.lineStyle(sw * 2.0, 0x111122, 1);
    g.lineBetween(ex - eR * 0.80, ey - eR * 0.50, ex, ey + eR * 0.18);
    g.lineBetween(ex + eR * 0.80, ey - eR * 0.50, ex, ey + eR * 0.18);
}
function smile(g, cx, cy, r, sw) {
...
function drawFacePluto(g, cx, cy, r, eLX, eRX, eY, eR, sw) {
...
function drawFaceMercury(g, cx, cy, r, eLX, eRX, eY, eR, sw) {
...
function drawFaceVenus(g, cx, cy, r, eLX, eRX, eY, eR, sw) {
    closedEyeHappy(g, eLX, eY, eR, sw);
    closedEyeHappy(g, eRX, eY, eR, sw);
    cheeks(g, cx, cy, r);
    smile(g, cx, cy, r, sw);
}
function drawFaceEarth(g, cx, cy, r, eLX, eRX, eY, eR, sw) {
    closedEyeHappy(g, eLX, eY, eR, sw);
    closedEyeHappy(g, eRX, eY, eR, sw);
    cheeks(g, cx, cy, r);
    // Big smile arc
...
function drawFaceMars(g, cx, cy, r, eLX, eRX, eY, eR, sw) {
    // Angry V brows
...
function drawFaceJupiter(g, cx, cy, r, eLX, eRX, eY, eR, sw) {
    // Both eyes laughing ><
    closedEyeWink(g, eLX, eY, eR, sw);
    closedEyeWink(g, eRX, eY, eR, sw);
    cheeks(g, cx, cy, r);
...
function drawFaceSaturn(g, cx, cy, r, eLX, eRX, eY, eR, sw) {
    closedEyeHappy(g, eLX, eY, eR, sw);
    closedEyeHappy(g, eRX, eY, eR, sw);
    cheeks(g, cx, cy, r);
    smile(g, cx, cy, r, sw);
}
function drawFaceUranus(g, cx, cy, r, eLX, eRX, eY, eR, sw) {
...
function drawFaceNeptune(g, cx, cy, r, eLX, eRX, eY, eR, sw) {
    closedEyeSleepy(g, eLX, eY, eR, sw);
    closedEyeSleepy(g, eRX, eY, eR, sw);
    // Eyelashes
...
function drawFaceSun(g, cx, cy, r, eLX, eRX, eY, eR, sw) {
    closedEyeHappy(g, eLX, eY, eR, sw);
    closedEyeHappy(g, eRX, eY, eR, sw);
    cheeks(g, cx, cy, r);
    smile(g, cx, cy, r, sw);
...
function drawSaturnRingsTexture(scene, config) {
    const r = config.radius;
...
function drawNeptuneRingsTexture(scene, config) {
    const r = config.radius;
...
export function generateSupernovaTexture(scene) {
    const size = 56;
