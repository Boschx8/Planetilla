...
export class GameScene extends Phaser.Scene {
    ...
    drawProgressionPanel() {
        ...
        const dispR = planets.map(p => Math.round(minDisp + (p.radius - minSrc) / (maxSrc - minSrc) * (maxDisp - minDisp)));
        const arrowW = 9;
    ...
    setupInput() {
        ...
        this.input.on('pointerdown', (ptr) => {
            ...
            if (ptr.wasTouch)
                return;
            if (this.blockNextDrop) {
                this.blockNextDrop = false;
                return;
            }
            if (this.canDrop && this.pendingPlanet)
                this.dropPlanet();
        });
    ...
    checkMerge(a, b) {
        ...
        if (planetA === this.pendingPlanet || planetB === this.pendingPlanet)
            return;
        if (planetA.planetId !== planetB.planetId)
            return;
        // Supernova planets don't merge
        if (planetA.planetId === SUPERNOVA_PLANET_ID)
            return;
        if (planetA.merging || planetB.merging)
            return;
        if (planetA.justSpawned || planetB.justSpawned)
            return;
        const idA = planetA.body.id;
        const idB = planetB.body.id;
        const key = `${Math.min(idA, idB)}-${Math.max(idA, idB)}`;
        if (this.mergeCooldowns.has(key))
            return;
        ...
        const doMerge = () => {
            ...
            for (const planet of this.planets) {
                if (planet.merging || planet === this.pendingPlanet || !planet.scene || planet.isStatic())
                    continue;
                const dx = planet.x - mx;
                const dy = planet.y - my;
                const dist = Math.sqrt(dx * dx + dy * dy);
                if (dist < 1 || dist > impulseRadius)
                    continue;
                const force = impulsePower * (1 - dist / impulseRadius);
                this.matter.applyForce(planet.body, { x: (dx / dist) * force, y: (dy / dist) * force });
            }
        };
        let completed = 0;
        const onDone = () => { if (++completed === 2)
            doMerge(); };
    ...
    getPlanetFromBody(body) {
        ...
        const bodyAny = body;
        const id = (bodyAny.parent ?? bodyAny).id;
        for (const planet of this.planets) {
            if (!planet.scene)
                continue;
            const pb = planet.body;
            if (pb?.id === id)
                return planet;
        }
        return null;
    ...
    spawnPendingPlanet() {
        ...
        const pmb = this.pendingPlanet.body;
        if (pmb)
            pmb.parts?.forEach((p) => { p.isSensor = true; });
    ...
    detonateActiveSupernovaAt() {
        if (!this.activeSupernovaPlanet || this.gameOverTriggered)
            return;
        ...
        for (const planet of this.planets) {
            if (planet === this.pendingPlanet || planet.merging || !planet.scene)
                continue;
            ...
            const dist = Math.sqrt(dx * dx + dy * dy);
            if (dist < 1 || dist >= SN_EXPLOSION_RADIUS)
                continue;
    ...
    updateInternal(delta) {
        ...
        let anyDanger = false;
        for (const planet of this.planets) {
            if (planet === this.pendingPlanet || planet.merging || !planet.scene || planet.justSpawned)
                continue;
            if (planet.planetId === SUPERNOVA_PLANET_ID)
                continue; // supernova doesn't trigger game over
            const inDanger = planet.y < DROP_ZONE_Y + 5;
            if (inDanger) {
                planet.dangerTime += delta;
                planet.setInDanger(true);
                anyDanger = true;
                if (planet.dangerTime > maxDangerTime)
                    maxDangerTime = planet.dangerTime;
    ...
    showGameOverUI() {
        ...
        if (finalScore > best)
            localStorage.setItem('planetilla_best', String(finalScore));
    ...
    _drawSadPlanetUI(cx, cy) {
        ...
        g.beginPath();
        g.arc(cx, cy + r * 0.33 + r * 0.22 * 0.55, r * 0.22, Math.PI + 0.2, Math.PI * 2 - 0.2, false);
        g.strokePath();
